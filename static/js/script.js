$(document).ready(function() {
    ...
    let incomingRingtone = new Audio('/static/assets/ringtone.mp3');
    ...
    $('#endActiveCall').off('click').click(function() {
        endCall();
    });
    ...
    function loadContacts() {
        $.get('/api/contacts').done(function(contacts) {
            ...
            if (!contacts || contacts.length === 0) {
                html = '<li class="list-group-item">No contacts yet. Search and add friends!</li>';
            } else {
                contacts.forEach(c => {
    ...
    function loadPendingRequests() {
        $.get('/api/pending_requests').done(function(requests) {
            ...
            if (!requests || requests.length === 0) {
                html = '<li class="list-group-item">No pending requests</li>';
            } else {
                requests.forEach(r => {
    ...
    function showLoader(show=true) {
        if (show) $('#globalLoader').css('display','flex');
        else $('#globalLoader').hide();
    }
    ...
    // Updated Delete account to use confirmation and loader
    $('#deleteAccount').off('click').click(function() {
        confirmAction('Are you sure you want to delete your account? This is permanent.', function() {
            showLoader(true);
            $.post('/api/delete_account').done(function() {
                showToast('Account deleted', 'success');
                window.location.href = '/';
    ...
    // Wrap API calls used widely to show loader + error handling
    function apiPostWithUI(url, data, successCb) {
        showLoader(true);
        $.ajax({ url: url, method: 'POST', contentType: 'application/json', data: JSON.stringify(data) })
        .done(function(resp) { if (successCb) successCb(resp); })
        .fail(function(xhr) { showToast((xhr.responseJSON && xhr.responseJSON.error) ? xhr.responseJSON.error : 'Error', 'error'); })
        .always(function() { showLoader(false); });
    }
    ...
    $('#contactList').on('click', '.block-user', function() {
        ...
        confirmAction('Block this user? You can unblock later.', function() {
            apiPostWithUI('/api/block_user', {contact_id: userId}, function() {
                showToast('User blocked', 'warning');
                loadContacts();
            });
        });
        ...
        const userId = $(this).data('id');
        confirmAction('Unblock this user?', function() {
            apiPostWithUI('/api/unblock_user', {contact_id: userId}, function() {
                showToast('User unblocked', 'success');
                loadContacts();
            });
        });
    ...
    // Render a contact card into #contactsGrid
    function renderContactCard(c) {
        const statusClass = c.online ? (c.in_call ? 'in-call' : 'online') : 'offline';
        const badge = c.online ? (c.in_call ? '<span class="badge bg-warning text-dark">In call</span>' : '<span class="badge bg-success">Online</span>') : '<span class="badge bg-secondary">Offline</span>';
        return `
        <div class="col-md-4 mb-3" data-id="${c.id}">
            <div class="card p-3 h-100 contact-card">
                <div class="d-flex justify-content-between align-items-start">
                    <div>
                        <div class="fw-bold">${c.username}</div>
                        <div class="small footer-note mt-1"><span class="online-indicator ${statusClass}"></span> ${c.online ? (c.in_call ? 'In call' : 'Online') : 'Offline'}</div>
                    </div>
                    <div>
                        ${badge}
                    </div>
                ...
                <div class="mt-3 d-flex gap-2">
                    <button class="btn btn-sm btn-success call-user" data-id="${c.id}" data-username="${c.username}">Call</button>
                    <button class="btn btn-sm btn-outline-light block-user" data-id="${c.id}">Block</button>
                </div>
            </div>
        </div>`;
    }

    function updateContactInGrid(contact) {
        const el = $(`#contactsGrid [data-id='${contact.id}']`);
        const html = renderContactCard(contact);
        if (el.length) {
            el.replaceWith(html);
            // subtle highlight animation
            const newEl = $(`#contactsGrid [data-id='${contact.id}']`);
            newEl.find('.card').css('box-shadow','0 0 0 4px rgba(99,102,241,0.12)').animate({opacity:1}, 600, function() { $(this).css('box-shadow',''); });
        } else {
            $('#contactsGrid').prepend(html);
        }
    }

    // Handle real-time presence events
    socket.on('user_presence', function(data) {
        // data: {user_id, online, in_call}
        // Update indicator if present in the grid
        const el = $(`#contactsGrid [data-id='${data.user_id}']`);
        if (el.length) {
            const username = el.find('.fw-bold').text();
            updateContactInGrid({id: data.user_id, username: username, online: data.online, in_call: data.in_call});
        }
    });

    // Debounced search
    $('#searchUser').off('input').on('input', debounce(function() {
        ...
        $.get('/api/search_users', {q: query}).done(function(users) {
            let html = '';
            users.forEach(u => {
                html += `<li class="list-group-item d-flex justify-content-between align-items-center" data-id="${u.id}" data-username="${u.username}">${u.username} <button class="btn btn-sm btn-primary add-contact">Add</button></li>`;
            });
            $('#searchResults').html(html);
        });
    }, 250));

    // When contacts load, render grid
    function loadContacts() {
        $.get('/api/contacts').done(function(contacts) {
            if (!contacts || contacts.length === 0) {
                $('#contactsGrid').html('<div class="col-12"><div class="card p-3">No contacts yet. Use the search box to add friends.</div></div>');
                return;
            }
            let html = '';
            contacts.forEach(c => { html += renderContactCard(c); });
            $('#contactsGrid').html(html);
        });
    }
});
