...
db = SQLAlchemy(app)
# Allow cross-origin WebSocket from local dev clients. Adjust in production.
socketio = SocketIO(app, cors_allowed_origins="*")

login_manager = LoginManager(app)

# Return JSON 401 for API endpoints instead of redirecting to HTML login page.
...
def unauthorized():
    try:
        ...
        pass
    # Otherwise, redirect to landing (HTML login)
    return redirect(url_for('landing'))


# Add common security headers for responses
@app.after_request
def set_security_headers(response):
    response.headers['X-Frame-Options'] = 'DENY'
    response.headers['X-Content-Type-Options'] = 'nosniff'
    response.headers['Referrer-Policy'] = 'no-referrer-when-downgrade'
    # Relaxed CSP to allow CDN assets used in templates. Tighten in production.
...
# Ensure DB and tables exist before serving requests
@app.before_first_request
def ensure_db():
    # If using sqlite, ensure file and tables exist. Create parent directory if necessary.
    ...
    try:
        if db_path:
            ...
            # Create file/tables if missing
            with app.app_context():
                # Create all tables if DB file not present
                if not os.path.exists(db_path):
                    app.logger.info('Database file missing; creating %s', db_path)
                    db.create_all()
                else:
                    # Verify required tables exist; create missing tables
                    insp = inspect(db.engine)
                    existing = set(insp.get_table_names())
                    required = {User.__tablename__, Contact.__tablename__}
                    if not required.issubset(existing):
                        app.logger.info('Database tables missing; creating missing tables')
                        db.create_all()
        else:
            # Non-sqlite DBs: ensure tables exist
            with app.app_context():
                insp = inspect(db.engine)
                existing = set(insp.get_table_names())
                required = {User.__tablename__, Contact.__tablename__}
                if not required.issubset(existing):
                    app.logger.info('Creating missing tables in database')
                    db.create_all()
    except Exception as e:
        app.logger.exception('Failed to ensure database: %s', e)
...
@login_required
def delete_account():
    # Remove all contact relations involving this user, then delete user and logout safely.
    db.session.query(Contact).filter((Contact.user_id == current_user.id) | (Contact.contact_id == current_user.id)).delete(synchronize_session=False)
    # Load the actual user instance from the session-bound ORM
    user = User.query.get(current_user.id)
    # Clear the login session first
    logout_user()
    if user:
...
def handle_connect():
    if current_user.is_authenticated:
        ...
        socketio.emit('user_presence', {'user_id': current_user.id, 'online': True, 'in_call': current_user.in_call}, broadcast=True, include_self=False)
    else:
        return False
...
def handle_disconnect():
    if current_user.is_authenticated:
        ...
        db.session.commit()
        # Broadcast presence update to other users
        socketio.emit('user_presence', {'user_id': current_user.id, 'online': False, 'in_call': False}, broadcast=True, include_self=False)
